...
const reportPath = join(workspace, "logs/comment_check_20260629_033300.json");
...
function classifyComment(message, authorName) {
  ...
  if (!text.trim()) return "spam";
  if (/https?:\/\/|zalo\.me|inbox\b|ib\b|liên hệ|lh\b|nhắn tin|gọi/.test(text)) {
    if (!authorName || /(bot|spam|ads)/i.test(authorName)) return "spam";
    return "xin tư vấn";
  }
  if (/giá|bao nhiêu|báo giá|quote|chi phí|tổng tiền/.test(text)) return "hỏi giá";
  if (/kỹ thuật|bảo trì|bảo dưỡng|lỗi|sửa|sự cố|rung|ồn|kẹt|nhiệt|điện|cứu hộ|bảo hành/.test(text)) return "hỏi kỹ thuật";
  if (/tư vấn|phù hợp|nhà phố|biệt thự|kích thước|hố thang|mẫu|so sánh|nên chọn/.test(text)) return "xin tư vấn";
  if (/liên hệ|lh|zalo|inbox|ib\b/.test(text)) return "xin tư vấn";
  return "xin tư vấn";
}
...
function buildReply(item) {
  ...
  if (/bảo trì|bảo dưỡng|lỗi|sửa|sự cố|rung|ồn|kẹt|cứu hộ|bảo hành|kỹ thuật/.test(lower)) {
  ...
  if (/tư vấn|phù hợp|nhà phố|biệt thự|kích thước|hố thang|mẫu|so sánh|nên chọn|thang máy/.test(lower)) {
    return "Dạ anh/chị có thể để lại số điện thoại hoặc inbox trực tiếp, FUJI TH sẽ tư vấn phương án thang máy phù hợp theo hiện trạng nhà mình ạ.";
  }
...
for (const post of posts.data || []) {
  ...
  for (const comment of comments.data || []) {
    ...
    if (action === "replied") {
      const message = buildReply(comment);
...
const report = {
  ...
  processed: newCommentItems.length,
  total_new: newCommentItems.length,
  items: newCommentItems,
  channels: {
    facebook: {
      page_id: pageId,
      posts_scanned: postsScanned,
      new_comments: newCommentItems.length,
    },
    website: {
  ...
  note: newCommentItems.length
    ? `Đã xử lý ${newCommentItems.length} bình luận Facebook mới; website ${websiteNewCount === 0 ? "không có bình luận mới" : websiteNewCount === -1 ? "không kiểm tra được" : `có ${websiteNewCount} bình luận`}.`
...
const state = {
  ...
  summary: {
    new_comments: newCommentItems.length,
    handled: newCommentItems.filter((i) => i.action === "replied").length,
  },
};
